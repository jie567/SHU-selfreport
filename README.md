...
在 `config.yaml` 中设置所有需要打卡的学号密码，如果人员来自不同校区还可以在其中设置特殊校区
...
`NAME` 设置为 `USERS`
`VALUE` 设置为 `学号1,密码1,姓名1（学号1）;学号2,密码2,姓名2（学号2）,校区2` 的格式，注意逗号与分号的区分，学号密码之间用逗号，每两个学号之间用分号，如果只有一个学号密码则不需要加分号，没有校区默认宝山校区
